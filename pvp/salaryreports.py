...
class SalaryReport():
    ...
    def __init__(self, compensation_summaries=[], pvp_evaluations=[]):
        self.total_salaries = Decimal('0.00')
        self.categories = {}
...
def get_salary_report_for_team(team_id):
    compensation_summaries = CompensationSummary.objects.get_summaries_for_team(team_id)
    current_round = EvaluationRound.objects.most_recent()
    pvp_evaluations = PvpEvaluation.objects.get_evaluations_for_team(team_id, current_round.id)
    return SalaryReport(compensation_summaries=compensation_summaries, pvp_evaluations=pvp_evaluations)