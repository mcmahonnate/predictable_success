...
class PvpEvaluationTest(TestCase):
    ...
    def test_get_most_recent(self):
        ...
        bob = Employee(full_name='Bob Doe', display=True)
        barb = Employee(full_name='Barb Doe', display=False)
        john.save()
        jane.save()
        bob.save()
        barb.save()
        ...
        pvp4 = PvpEvaluation(employee=bob, evaluation_round=old_round, potential=1, performance=1).save()
        pvp5 = PvpEvaluation(employee=barb, evaluation_round=new_round, potential=1, performance=1).save()
        pvps = PvpEvaluation.objects.get_most_recent_for_employees([john, jane, barb])
